...
- 并行构建：四个项目镜像后台并行 + `wait` 全部 PID；失败以退出码与最终镜像存在为准。
- 跳过拉取：仅对非项目运行镜像做 `docker image inspect` 命中判断，项目镜像始终重建。
- 端口存活：进程参数匹配 + 目标端口真实可连才算"已在运行"，否则清理残留并重建转发。

## 5. 影响范围

| 模块 | 影响 |
| --- | --- |
| hack/local-cluster.sh | 并行构建、跳过重复拉取、端口转发存活检查 |
| 脚本执行位 | local-cluster.sh 等恢复 100755 并提交 mode |
| 部署清单 / CRD / 数据库 | 无变化 |
...
- 多副本 Backend 行为（本机为单副本）。
- 低配机器（<4 CPU）下并行构建的实际收益。
