...
数据库生命周期改为"自动 + 可感知"：Backend 连接/迁移失败按退避重试（默认 6 次 × 5s，可配），启动日志明确打印已应用迁移数与历史数据量（含是否恢复历史）；Backend 部署增加 initContainer 等待 PostgreSQL 就绪，消除 CrashLoop；健康接口新增 `database` 详情（迁移数/事件数/快照数/当前态数）。当前态结构化入库：快照循环把聚合快照中的每个资源（Model/Tenant/WorkerNode/策略/实例/Traffic/Workloads）拆成 `resource_states` 行周期性 upsert，数据库成为所有展示数据的持久化副本。
...
flowchart LR
  ...
  DB -->|当前态查询基础 ListResourceStates| API
```
...
- 查询基础：`ListResourceStates(kind, namespace, limit)`（Store 层，HTTP 查询接口留待 Phase 3 与读路径一起做）。
...
| Backend API | 健康接口新增 `checks.database` 详情 |
...
## 6. 明确未实现（Phase 3，另行设计）

- 前端当前态读路径仍走 informer cache，未迁移到数据库查询。
- `resource_states` 尚无 HTTP 查询接口。
- 时间线事件来源已基于数据库（既有实现），未改动。