# WSL 回环研究：openvmm/WSL “为什么这么写”git 溯源 + 版本缺陷区间勘误
...
- 溯源过程中发现既有研究文档（27/28/29 号）的版本缺陷区间归因有误：#40187 被错误归为“2.7.8.0 不含”，且 #41125/#41051 的首发版本被误写为“2.7.11 起”。勘误直接改变研究叙事锚点，需立即沉淀并同步面试详案。
...
2. **版本缺陷区间勘误**：
   - #40187（inline 端口 0 解析）在 2.7.8.0 guest 侧 tracker 中**已含**（guest 侧文件，不在 host DLL）——修正对应研究文档。
...
3. **文档同步**：新增本地研究文档（Desktop/WSL + 仓库 Documents 镜像）；对应研究文档追加勘误节；面试详案版本表述修正（Documents/ 本地副本）。
...
- 本地研究文档已写 Desktop/WSL 与仓库 Documents/（gitignore 不入库），对应勘误同步完成。
- 面试详案三处替换全部命中，“缺 #40187”残留 0 处。
- 未验证：2.9.5+ 升级后探针成功率恢复（闭环动作，需用户拍板装新版 WSL）；guest 侧 pidfd 日志（超出本机可观测范围）。

## 回滚

- 纯研究沉淀：删除本地研究文档 + 各勘误节即可回滚，零运行时风险；仓库侧条目删除同目录即可。
