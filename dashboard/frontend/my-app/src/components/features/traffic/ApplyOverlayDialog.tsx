        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto border-white/[0.08] bg-[#0A0E15] p-0 text-[#E8EEF7] shadow-2xl">
                <DialogHeader className="border-b border-white/[0.06] px-6 pb-4 pt-5">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-[14px] text-[#657184]">{template?.name ?? '未选择模板'} · 纯 QPS 加法</p>
                        </div>
                    </div>
                </DialogHeader>
                ...
                <div className="grid grid-cols-1 gap-0 lg:grid-cols-[340px_minmax(0,1fr)]">
                    <div className="border-b border-white/[0.06] px-6 py-5 lg:border-b-0 lg:border-r">
                        ...
                        <div className="mb-5 rounded-xl border border-[#5B8CFF]/15 bg-[#5B8CFF]/[0.045] p-3">
                            <div className="flex items-center justify-between gap-3 text-[13px] text-[#61708A]">
                                <span>场景基准切面</span>
                                <span className={replay.mode === 'historical' ? 'text-[#A8B5FF]' : 'text-emerald-300'}>
                                    {replay.mode === 'historical' ? '历史' : '最新'}
                                </span>
                            </div>
                            <div className="mt-1.5 truncate font-mono text-[12px] text-[#AAB8CC]">
                                {formatUtcTimestamp(replay.effectiveAt, true)} UTC
                            </div>
                            <div className="mt-1 truncate font-mono text-[12px] text-[#4F5B70]">
                        ...
                        <div className="space-y-5">
                            <div className="space-y-2">
                                <Label className="flex items-center gap-1.5 text-[14px] font-medium text-[#8C98AA]">
                                        ...
                                        {tenants.map((tenant) => (
                                            <SelectItem key={tenant.id} value={tenant.id}>
                                                <span className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[13px] text-[#667286]">{tenant.priority}</span>
                                                </span>
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                ...
                                {attempted && !tenantId && (
                                    <p className="text-[12px] text-amber-300">请选择模板实际归属的租户</p>
                                )}
                            </div>

                            <div className="space-y-2">
                                <Label htmlFor="traffic-offset" className="flex items-center gap-1.5 text-[14px] font-medium text-[#8C98AA]">
                                ...
                                <div className="relative">
                                    ...
                                    />
                                    <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[12px] text-[#5F6B7E]">秒</span>
                                </div>
                                {attempted && offsetSeconds === null && (
                                    <p className="text-[12px] text-amber-300">请输入大于或等于 0 的逻辑时间</p>
                                )}
                                <p className="text-[12px] leading-4 text-[#566174]">T+0 表示场景起点；模板持续时间由绘制坐标自动决定。</p>
                            </div>
                        </div>

                        <div className="mt-6 rounded-xl border border-white/[0.06] bg-white/[0.018] p-3">
                            <div className="flex items-center gap-2 text-[12px] text-[#697588]">
                            ...
                            <div className="mt-2 flex items-center gap-2 text-[12px] text-[#7D899B]">
                                <span>原流量</span>
                                <MoveRight className="h-3 w-3" />
                                <span className="text-[#8BBCFF]">原流量 + 模板增量</span>
                            </div>
                            {selectedTenant && (
                                <div className="mt-3 border-t border-white/[0.06] pt-3 text-[12px] leading-4 text-[#7D899B]">
                                    <div className="flex items-center justify-between">
                                        <span>应用后租户目标 QPS</span>
                                        <span className="font-mono text-[#8BBCFF]">{formatQps(targetQps)}</span>
                                    </div>
                                    <p className="mt-1.5 text-[13px] text-[#55617A]">
                    ...
                    <div className="min-h-[350px] px-5 py-5 lg:min-h-[430px]">
                        <div className="mb-3 flex items-center justify-between">
                            <div>
                                ...
                                <div className="mt-1 text-[12px] text-[#5F6B7E]">横轴始终使用从 0 开始的逻辑秒</div>
                            </div>
                            {offsetSeconds !== null && (
                                <span className="rounded-full border border-[#5B8CFF]/20 bg-[#5B8CFF]/8 px-2 py-1 font-mono text-[13px] text-[#7FB4FF]">
                ...
                <DialogFooter className="border-t border-white/[0.06] px-6 py-4">
                    {historical && (
                        <p className="mr-auto text-[12px] text-amber-300">
                            历史模式只读，不能应用流量
                        </p>
                    )}
                    <Button
...
function Metric({ label, value }: { label: string; value: string }) {
    return (
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2.5">
            ...
            <div className="mt-1 font-mono text-[14px] font-medium text-[#C9D5E3]">{value}</div>
        </div>
    )
}
