...
原审查识别了 3 个 P0 问题。调度选点执行契约和 Model 能力分输入路径已于 2026-08-14 完成修复；Backend 写接口的可信身份边界仍是未处理的 P0。其余 7 个 P1 问题应在扩大租户、节点和数据保留规模前按依赖顺序推进。

完整排序见 [TOP10_ISSUES.md](TOP10_ISSUES.md)。建议先阅读 P0，再处理 CRD 契约和命令一致性，最后推进规模化、历史数据和交付门禁。

## 与 GitHub Issue / 看板的关联（2026-08-16 更新）

本目录是静态审查事实源，不承载执行状态。条目进入执行闭环的路径：

- 每条问题按需提取为 GitHub Issue（标题遵循仓库模板 `design:` / `bug:` / `feat:`），正文首段引用对应 `project-review/issue-NN-*.md`。
- Issue 通过 `gh project item-add` 链接到 GitHub Project v2 看板 **Project Review**，由 Status 字段（To do → In review → Approved → In progress → Done）驱动生命周期。
- 交付后提交信息用 `Fixes #N` 自动关闭 issue，卡片移入 Done 并归档；本目录不修改，作为问题背景的历史基线。
- 详细关联模型与批量闭环规则见 `docs/agents/PROJECT_REVIEW.md`。
