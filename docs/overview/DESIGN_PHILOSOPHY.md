...
AIOps 的 LLM 输出（实体总结、分数、窗口/日认知、警戒、对话回答）只写入 `aiops_*` 表，供人阅读与辅助操作；它不写 CR、不驱动 Controller、不参与调度决策。硬指标规则先行、schema 校验、失败重试与规则兜底，保证单次 LLM 故障不会污染结论；分析入队由运行时开关与 `AIOPS_ENABLED` 双重控制，LLM 依赖缺失时系统其余部分完整可用。

为什么重要：把外部不可控的 LLM 放进系统的唯一安全方式是让它只读历史数据、只产生可审计的辅助结论，并把“落地执行”留给用户确认后的既有写通道（`/aiops/commands` 的 confirm gate）。
