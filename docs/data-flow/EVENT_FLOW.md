...
切面 complete/fail 时 Backend 将分析任务入队（`aiops_analyses`，幂等）；AIOps worker 按状态机推进（pending→running→aggregating→completed/failed），L1 实体总结与 L2 分数写入 `aiops_*` 表。该链路异步、只读切面数据，不阻塞实验生命周期；分析失败在 `error_text` 记录，未达重试上限（`AIOPS_MAX_ATTEMPTS_PER_ANALYSIS`）回 `pending` 下轮重试，达上限落 `failed`，前端可展示尝试次数与失败原因。
