...
| GET | `/aiops/limits` | 意图执行硬限制与能力（峰值 QPS/倍速/波形/潮汐周期/时长不限/随时可停止），解析校验与前端提示共用。 |
| POST | `/aiops/commands/{id}/stop` | 停止执行中的波形调度（#134）：QPS 归零 + 恢复倍速 + 状态 stopped。 |
| GET | `/aiops/quota` | 日配额用量与上限（#134）：24h 调用/token 已用与上限，面板展示剩余额度。 |
