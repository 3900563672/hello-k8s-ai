...
AIOps 是 Dashboard 内置的 AI 辅助分析能力，用自然语言把切面实验的数据变成结论，不做控制面之外的任何写入。

主要职责：

- 浮窗对话：用户在 Dashboard 右下角浮窗提问（调度、指标、Trace、租户/节点/Pod 状态）；
- 分层总结：按切面（租户、节点、Pod、调度等）异步总结历史数据，结果结构化沉淀到数据库，避免把原始数据全部塞进上下文；
- 意图执行：识别用户意图（如设置目标 QPS、调整倍速），只走白名单写接口；
- 时间聚合与警戒：按时间窗口（如两小时/每日）聚合，分数持续偏低时产生警戒提示；
- 运行时开关：设置面板可关闭 AI 分析（实验照常运行，但不入队分析，节省 LLM 预算）。

实现说明：

- Backend 分层总结服务，OpenAI 兼容 API（Base URL / Model / Key 可配，Key 存 Kubernetes Secret）；
- 前端浮窗挂在 Dashboard 布局上，全站可用；设置面板配置 Key 并控制开关；
- 分析结果与对话历史持久化在 PostgreSQL（aiops_* 表），与业务快照同源可追溯。
